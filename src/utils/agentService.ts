...
【你的核心能力 - 项目全局掌控】
- 你能读取用户在所有9颗行星的完整数据
- 你能访问用户的记忆宫殿、任务中心、数据分析
- 你能进行跨模块分析，发现不同行星数据之间的关联
- 你能生成用户宇宙的完整叙事和洞察
- 你能推荐用户还未探索的功能
- 你能主动关怀用户的状态
...
1. 导航：<<NAVIGATE:路径>>
   路径：/mercury, /venus, /mars, /jupiter, /saturn, /uranus, /neptune, /sun, /earth, /observatory, /agent, /oracle-cards, /breathing, /reflection, /inner-child, /

2. 记录到行星：<<RECORD:行星|内容>>
   行星：mercury(思绪), mars(情绪), venus(信), jupiter(贵人), saturn(节点), neptune(梦), sun(愿望), earth(感受)
...
function fallbackResponse(userMessage: string, history: { role: string; content: string }[]): string {
  ...
  // 全局掌控能力
  // 检查宇宙叙事
  if (msg.includes('宇宙') && (msg.includes('故事') || msg.includes('叙事') || msg.includes('怎么样') || msg.includes('状态'))) {
    return generateCosmicNarrative()
  }

  // 检查跨模块洞察
  if (msg.includes('洞察') || msg.includes('发现') || msg.includes('关联')) {
    const insights = getCrossModuleInsights()
    if (insights.length === 0) return '还没有足够的跨模块数据，继续探索宇宙吧 ✨'
    return `🔮 发现了一些关联：\n\n${insights.slice(0, 2).map((i) => `• ${i.title}\n  ${i.description}\n  → ${i.action}`).join('\n')}`
  }

  // 检查功能推荐
  if (msg.includes('推荐') && (msg.includes('功能') || msg.includes('探索'))) {
    const recs = getFeatureRecommendations()
    if (recs.length === 0) return '你已经探索了很多功能！试试组合炼金吧 ✨'
    return `✨ 我推荐你试试：\n\n${recs.slice(0, 2).map((r) => `【${r.category}】\n${r.features.slice(0, 2).map((f) => `• ${f.name}: ${f.reason}`).join('\n')}`).join('\n')}`
  }

  // 检查项目总览
  if (msg.includes('总览') || msg.includes('整体') || msg.includes('全局')) {
    const status = getProjectStatus()
    return `🌌 你的宇宙总览：

• 总记录: ${status.totalRecords} 条星光
• 探索进度: ${status.explorationProgress}%（${Math.round(status.explorationProgress * 9 / 100)}/9颗行星）
• 星灵等级: ${status.level}（${status.exp}星尘）
${status.streak > 0 ? `• 连续 ${status.streak} 天完成任务\n` : ''}${status.recentActivity.length > 0 ? `• ${status.recentActivity[0]}` : ''}

${status.untappedFeatures.length > 0 ? `\n未探索：${status.untappedFeatures.join('、')}` : ''}`
  }
...
export async function chatWithStarSpirit(
  ...
  const projectSummary = `【项目全局状态】
• 总星光: ${projectStatus.totalRecords} 条
• 探索进度: ${projectStatus.explorationProgress}% (${Math.round(projectStatus.explorationProgress * 9 / 100)}/9行星)
• 星灵等级: ${projectStatus.level} (${projectStatus.exp}星尘)
${projectStatus.streak > 0 ? `• 连续 ${projectStatus.streak} 天\n` : ''}${projectStatus.untappedFeatures.length > 0 ? `• 未探索: ${projectStatus.untappedFeatures.join('、')}` : ''}`

  // 获取跨模块洞察
  const crossInsights = getCrossModuleInsights()
  const insightsSummary = crossInsights.length > 0
    ? `【跨模块洞察】\n${crossInsights.slice(0, 2).map((i) => `• ${i.title}: ${i.description}`).join('\n')}`
    : ''

  const systemPrompt = STAR_SPIRIT_SYSTEM + '\n\n' + projectSummary + '\n\n【行星数据】\n' + dataSummary + '\n\n【记忆宫殿】\n' + memorySummary + '\n\n【任务中心】\n' + taskSummary + '\n\n' + insightsSummary + TOOLS_PROMPT
