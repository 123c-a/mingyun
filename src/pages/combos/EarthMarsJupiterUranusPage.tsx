...
const config = comboConfigs['earth-mars-jupiter-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '事业机会', icon: '🌌' },
  { id: 'm2', name: '能力突破', icon: '🌟' },
  { id: 'm3', name: '创新工作', icon: '💫' },
  { id: 'm4', name: '快速成长', icon: '✨' },
]
...
export default function EarthMarsJupiterUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('emju-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('emju-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('emju-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('emju-items4', [])
