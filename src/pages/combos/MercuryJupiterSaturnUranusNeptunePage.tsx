...
const config = comboConfigs['mercury-jupiter-saturn-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '身体觉察', icon: '🌌' },
  { id: 'm2', name: '情绪流动', icon: '🌟' },
  { id: 'm3', name: '心智成长', icon: '💫' },
  { id: 'm4', name: '整合修行', icon: '✨' },
]
...
export default function MercuryJupiterSaturnUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('merc-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('merc-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('merc-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('merc-items4', [])
