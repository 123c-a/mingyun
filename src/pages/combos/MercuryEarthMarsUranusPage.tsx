...
const config = comboConfigs['mercury-earth-mars-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '闪电灵感', icon: '🌌' },
  { id: 'm2', name: '创新执行', icon: '🌟' },
  { id: 'm3', name: '打破常规', icon: '💫' },
  { id: 'm4', name: '快速迭代', icon: '✨' },
]
...
export default function MercuryEarthMarsUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('memu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('memu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('memu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('memu-items4', [])
