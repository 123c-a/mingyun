...
const config = comboConfigs['venus-earth-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '心的打开', icon: '🌌' },
  { id: 'm2', name: '觉醒之爱', icon: '🌟' },
  { id: 'm3', name: '自由心灵', icon: '💫' },
  { id: 'm4', name: '觉醒时刻', icon: '✨' },
]
...
export default function VenusEarthUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('veun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('veun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('veun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('veun-items4', [])
