...
const config = comboConfigs['earth-jupiter-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '心灵财富', icon: '🌌' },
  { id: 'm2', name: '内在价值', icon: '🌟' },
  { id: 'm3', name: '丰盛根基', icon: '💫' },
  { id: 'm4', name: '精神富足', icon: '✨' },
]
...
export default function EarthJupiterSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('ejsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('ejsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('ejsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('ejsn-items4', [])
