...
const config = comboConfigs['venus-mars-jupiter-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '热情爆发', icon: '🌌' },
  { id: 'm2', name: '关系突破', icon: '🌟' },
  { id: 'm3', name: '创意之爱', icon: '💫' },
  { id: 'm4', name: '自由热恋', icon: '✨' },
]
...
export default function VenusMarsJupiterUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vmju-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vmju-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vmju-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vmju-items4', [])
