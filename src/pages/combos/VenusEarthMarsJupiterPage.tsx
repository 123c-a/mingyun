...
const config = comboConfigs['venus-earth-mars-jupiter']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '爱的行动', icon: '🌌' },
  { id: 'm2', name: '丰盛创造', icon: '🌟' },
  { id: 'm3', name: '热情分享', icon: '💫' },
  { id: 'm4', name: '幸福行动', icon: '✨' },
]
...
export default function VenusEarthMarsJupiterPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vemj-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vemj-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vemj-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vemj-items4', [])
