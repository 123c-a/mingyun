...
const config = comboConfigs['mercury-jupiter-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '意识扩展', icon: '🌌' },
  { id: 'm2', name: '宇宙视角', icon: '🌟' },
  { id: 'm3', name: '合一体验', icon: '💫' },
  { id: 'm4', name: '无限思维', icon: '✨' },
]
...
export default function MercuryJupiterUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mjun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mjun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mjun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mjun-items4', [])
