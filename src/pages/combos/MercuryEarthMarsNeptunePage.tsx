...
const config = comboConfigs['mercury-earth-mars-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '梦想蓝图', icon: '🌌' },
  { id: 'm2', name: '落地步骤', icon: '🌟' },
  { id: 'm3', name: '直觉行动', icon: '💫' },
  { id: 'm4', name: '显化记录', icon: '✨' },
]
...
export default function MercuryEarthMarsNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('memn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('memn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('memn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('memn-items4', [])
