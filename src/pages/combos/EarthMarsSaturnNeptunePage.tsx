...
const config = comboConfigs['earth-mars-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '压力成长', icon: '🌌' },
  { id: 'm2', name: '职业深度', icon: '🌟' },
  { id: 'm3', name: '坚持与梦想', icon: '💫' },
  { id: 'm4', name: '深海价值', icon: '✨' },
]
...
export default function EarthMarsSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('emsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('emsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('emsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('emsn-items4', [])
