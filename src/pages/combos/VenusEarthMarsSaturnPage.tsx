...
const config = comboConfigs['venus-earth-mars-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '守护清单', icon: '🌌' },
  { id: 'm2', name: '承诺行动', icon: '🌟' },
  { id: 'm3', name: '坚定温柔', icon: '💫' },
  { id: 'm4', name: '长期陪伴', icon: '✨' },
]
...
export default function VenusEarthMarsSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vems-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vems-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vems-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vems-items4', [])
