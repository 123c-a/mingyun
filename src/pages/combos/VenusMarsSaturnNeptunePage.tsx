...
const config = comboConfigs['venus-mars-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '深度激情', icon: '🌌' },
  { id: 'm2', name: '灵魂之约', icon: '🌟' },
  { id: 'm3', name: '转化之火', icon: '💫' },
  { id: 'm4', name: '深情力量', icon: '✨' },
]
...
export default function VenusMarsSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vmsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vmsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vmsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vmsn-items4', [])
