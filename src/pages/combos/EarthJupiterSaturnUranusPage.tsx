...
const config = comboConfigs['earth-jupiter-saturn-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '财富认知', icon: '🌌' },
  { id: 'm2', name: '收入突破', icon: '🌟' },
  { id: 'm3', name: '资产结构', icon: '💫' },
  { id: 'm4', name: '财富跃迁', icon: '✨' },
]
...
export default function EarthJupiterSaturnUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('ejsu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('ejsu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('ejsu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('ejsu-items4', [])
