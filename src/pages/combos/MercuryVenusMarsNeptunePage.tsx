...
const config = comboConfigs['mercury-venus-mars-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '灵魂共鸣', icon: '🌌' },
  { id: 'm2', name: '深层感受', icon: '🌟' },
...
export default function MercuryVenusMarsNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvmn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvmn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvmn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvmn-items4', [])
