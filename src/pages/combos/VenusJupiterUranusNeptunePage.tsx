...
const config = comboConfigs['venus-jupiter-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '宇宙之爱', icon: '🌌' },
  { id: 'm2', name: '无限温柔', icon: '🌟' },
  { id: 'm3', name: '灵性连接', icon: '💫' },
  { id: 'm4', name: '合一体验', icon: '✨' },
]
...
export default function VenusJupiterUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vjun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vjun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vjun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vjun-items4', [])
