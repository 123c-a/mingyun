...
const config = comboConfigs['mercury-jupiter-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '信念识别', icon: '🌌' },
  { id: 'm2', name: '信念结构', icon: '🌟' },
  { id: 'm3', name: '直觉验证', icon: '💫' },
  { id: 'm4', name: '信念成长', icon: '✨' },
]
...
export default function MercuryJupiterSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mjsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mjsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mjsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mjsn-items4', [])
