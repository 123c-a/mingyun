...
const config = comboConfigs['mercury-venus-earth-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '功课识别', icon: '🌌' },
  { id: 'm2', name: '学习记录', icon: '🌟' },
  { id: 'm3', name: '时间智慧', icon: '💫' },
  { id: 'm4', name: '成长里程碑', icon: '✨' },
]
...
export default function MercuryVenusEarthSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mves-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mves-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mves-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mves-items4', [])
