...
const config = comboConfigs['venus-earth-jupiter-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '幸福基石', icon: '🌌' },
  { id: 'm2', name: '感恩习惯', icon: '🌟' },
  { id: 'm3', name: '美好积累', icon: '💫' },
  { id: 'm4', name: '长久幸福', icon: '✨' },
]
...
export default function VenusEarthJupiterSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vejs-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vejs-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vejs-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vejs-items4', [])
