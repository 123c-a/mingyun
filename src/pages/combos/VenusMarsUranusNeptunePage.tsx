...
const config = comboConfigs['venus-mars-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '共振时刻', icon: '🌌' },
  { id: 'm2', name: '频率对齐', icon: '🌟' },
  { id: 'm3', name: '灵性共舞', icon: '💫' },
  { id: 'm4', name: '合一之爱', icon: '✨' },
]
...
export default function VenusMarsUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vmun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vmun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vmun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vmun-items4', [])
