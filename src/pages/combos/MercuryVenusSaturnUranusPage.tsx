...
const config = comboConfigs['mercury-venus-saturn-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '旧结构', icon: '🌌' },
  { id: 'm2', name: '打破时刻', icon: '🌟' },
  { id: 'm3', name: '新框架', icon: '💫' },
  { id: 'm4', name: '迭代进化', icon: '✨' },
]
...
export default function MercuryVenusSaturnUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvsu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvsu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvsu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvsu-items4', [])
