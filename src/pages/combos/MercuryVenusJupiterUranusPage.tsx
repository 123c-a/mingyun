...
const config = comboConfigs['mercury-venus-jupiter-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '灵感捕捉', icon: '🌌' },
  { id: 'm2', name: '创意突破', icon: '🌟' },
  { id: 'm3', name: '思维跳跃', icon: '💫' },
  { id: 'm4', name: '创造实验', icon: '✨' },
]
...
export default function MercuryVenusJupiterUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvju-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvju-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvju-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvju-items4', [])
