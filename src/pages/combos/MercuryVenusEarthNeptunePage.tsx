...
const config = comboConfigs['mercury-venus-earth-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '梦境记录', icon: '🌌' },
  { id: 'm2', name: '现实锚点', icon: '🌟' },
  { id: 'm3', name: '直觉落地', icon: '💫' },
  { id: 'm4', name: '心灵花园', icon: '✨' },
]
...
export default function MercuryVenusEarthNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mven-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mven-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mven-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mven-items4', [])
