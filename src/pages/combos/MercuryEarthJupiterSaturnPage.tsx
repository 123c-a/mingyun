...
const config = comboConfigs['mercury-earth-jupiter-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '学习路径', icon: '🌌' },
  { id: 'm2', name: '知识体系', icon: '🌟' },
  { id: 'm3', name: '实践应用', icon: '💫' },
  { id: 'm4', name: '长期成长', icon: '✨' },
]
...
export default function MercuryEarthJupiterSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mejs-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mejs-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mejs-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mejs-items4', [])
