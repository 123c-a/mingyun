...
const config = comboConfigs['mercury-earth-mars-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '长期目标', icon: '🌌' },
  { id: 'm2', name: '每日行动', icon: '🌟' },
  { id: 'm3', name: '耐心培养', icon: '💫' },
  { id: 'm4', name: '稳固成长', icon: '✨' },
]
...
export default function MercuryEarthMarsSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mems-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mems-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mems-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mems-items4', [])
