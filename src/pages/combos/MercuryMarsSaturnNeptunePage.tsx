...
const config = comboConfigs['mercury-mars-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '压力行动', icon: '🌌' },
  { id: 'm2', name: '持续燃烧', icon: '🌟' },
  { id: 'm3', name: '方向寻找', icon: '💫' },
  { id: 'm4', name: '深海成长', icon: '✨' },
]
...
export default function MercuryMarsSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mmsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mmsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mmsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mmsn-items4', [])
