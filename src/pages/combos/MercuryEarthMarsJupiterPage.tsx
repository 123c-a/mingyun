...
const config = comboConfigs['mercury-earth-mars-jupiter']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '行动清单', icon: '🌌' },
  { id: 'm2', name: '扩张计划', icon: '🌟' },
  { id: 'm3', name: '执行效率', icon: '💫' },
  { id: 'm4', name: '成长突破', icon: '✨' },
]
...
export default function MercuryEarthMarsJupiterPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('memj-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('memj-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('memj-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('memj-items4', [])
