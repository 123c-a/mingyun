...
const config = comboConfigs['earth-jupiter-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '未来愿景', icon: '🌌' },
  { id: 'm2', name: '创新现实', icon: '🌟' },
  { id: 'm3', name: '梦想落地', icon: '💫' },
  { id: 'm4', name: '理想生活', icon: '✨' },
]
...
export default function EarthJupiterUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('ejun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('ejun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('ejun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('ejun-items4', [])
