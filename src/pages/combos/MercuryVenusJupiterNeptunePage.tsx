...
const config = comboConfigs['mercury-venus-jupiter-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '感受翻译', icon: '🌌' },
  { id: 'm2', name: '灵性洞见', icon: '🌟' },
  { id: 'm3', name: '艺术表达', icon: '💫' },
  { id: 'm4', name: '心灵书写', icon: '✨' },
]
...
export default function MercuryVenusJupiterNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvjn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvjn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvjn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvjn-items4', [])
