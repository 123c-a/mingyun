...
const config = comboConfigs['mercury-mars-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '灵感火花', icon: '🌌' },
  { id: 'm2', name: '创意孵化', icon: '🌟' },
  { id: 'm3', name: '突破诞生', icon: '💫' },
  { id: 'm4', name: '蜕变记录', icon: '✨' },
]
...
export default function MercuryMarsUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mmun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mmun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mmun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mmun-items4', [])
