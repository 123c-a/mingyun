...
const config = comboConfigs['mercury-venus-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '边界觉察', icon: '🌌' },
  { id: 'm2', name: '温柔坚定', icon: '🌟' },
  { id: 'm3', name: '爱的学习', icon: '💫' },
  { id: 'm4', name: '深度滋养', icon: '✨' },
]
...
export default function MercuryVenusSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvsn-items4', [])
