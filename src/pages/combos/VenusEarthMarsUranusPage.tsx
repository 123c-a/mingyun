...
const config = comboConfigs['venus-earth-mars-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '勇敢表达', icon: '🌌' },
  { id: 'm2', name: '打破常规', icon: '🌟' },
  { id: 'm3', name: '自由去爱', icon: '💫' },
  { id: 'm4', name: '关系创新', icon: '✨' },
]
...
export default function VenusEarthMarsUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vemu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vemu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vemu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vemu-items4', [])
