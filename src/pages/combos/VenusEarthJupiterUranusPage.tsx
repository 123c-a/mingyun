...
const config = comboConfigs['venus-earth-jupiter-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '旧幸福观', icon: '🌌' },
  { id: 'm2', name: '新快乐定义', icon: '🌟' },
  { id: 'm3', name: '创新幸福', icon: '💫' },
  { id: 'm4', name: '自由快乐', icon: '✨' },
]
...
export default function VenusEarthJupiterUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('veju-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('veju-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('veju-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('veju-items4', [])
