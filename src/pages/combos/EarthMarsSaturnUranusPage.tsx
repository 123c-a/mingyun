...
const config = comboConfigs['earth-mars-saturn-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '旧职业观', icon: '🌌' },
  { id: 'm2', name: '职业突破', icon: '🌟' },
  { id: 'm3', name: '新工作方式', icon: '💫' },
  { id: 'm4', name: '职业重生', icon: '✨' },
]
...
export default function EarthMarsSaturnUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('emsu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('emsu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('emsu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('emsu-items4', [])
