...
const config = comboConfigs['earth-mars-jupiter-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '热爱事业', icon: '🌌' },
  { id: 'm2', name: '梦想工作', icon: '🌟' },
  { id: 'm3', name: '热情职业', icon: '💫' },
  { id: 'm4', name: '使命召唤', icon: '✨' },
]
...
export default function EarthMarsJupiterNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('emjn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('emjn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('emjn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('emjn-items4', [])
