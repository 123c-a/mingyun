...
const config = comboConfigs['mercury-venus-mars-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '打破模板', icon: '🌌' },
  { id: 'm2', name: '自由表达', icon: '🌟' },
  { id: 'm3', name: '创新相处', icon: '💫' },
  { id: 'm4', name: '觉醒关系', icon: '✨' },
]
...
export default function MercuryVenusMarsUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvmu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvmu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvmu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvmu-items4', [])
