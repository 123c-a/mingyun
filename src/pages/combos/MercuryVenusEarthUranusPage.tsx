...
const config = comboConfigs['mercury-venus-earth-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '觉察时刻', icon: '🌌' },
  { id: 'm2', name: '打破惯性', icon: '🌟' },
  { id: 'm3', name: '觉醒练习', icon: '💫' },
  { id: 'm4', name: '日常革命', icon: '✨' },
]
...
export default function MercuryVenusEarthUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mveu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mveu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mveu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mveu-items4', [])
