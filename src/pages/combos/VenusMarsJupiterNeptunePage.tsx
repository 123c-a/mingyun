...
const config = comboConfigs['venus-mars-jupiter-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '灵魂吸引', icon: '🌌' },
  { id: 'm2', name: '灵性热情', icon: '🌟' },
  { id: 'm3', name: '合一之火', icon: '💫' },
  { id: 'm4', name: '神圣之爱', icon: '✨' },
]
...
export default function VenusMarsJupiterNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vmjn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vmjn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vmjn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vmjn-items4', [])
