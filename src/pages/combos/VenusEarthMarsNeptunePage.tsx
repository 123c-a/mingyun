...
const config = comboConfigs['venus-earth-mars-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '爱的梦境', icon: '🌌' },
  { id: 'm2', name: '温柔行动', icon: '🌟' },
  { id: 'm3', name: '心之所向', icon: '💫' },
  { id: 'm4', name: '深情时刻', icon: '✨' },
]
...
export default function VenusEarthMarsNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vemn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vemn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vemn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vemn-items4', [])
