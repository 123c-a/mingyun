...
const config = comboConfigs['venus-jupiter-saturn-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '好运捕捉', icon: '🌌' },
  { id: 'm2', name: '丰盛突破', icon: '🌟' },
  { id: 'm3', name: '机会创造', icon: '💫' },
  { id: 'm4', name: '幸运结构', icon: '✨' },
]
...
export default function VenusJupiterSaturnUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vjsu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vjsu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vjsu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vjsu-items4', [])
