...
const config = comboConfigs['mercury-venus-earth-jupiter']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '美好发现', icon: '🌌' },
  { id: 'm2', name: '感恩播种', icon: '🌟' },
  { id: 'm3', name: '富足练习', icon: '💫' },
  { id: 'm4', name: '分享丰盛', icon: '✨' },
]
...
export default function MercuryVenusEarthJupiterPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvej-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvej-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvej-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvej-items4', [])
