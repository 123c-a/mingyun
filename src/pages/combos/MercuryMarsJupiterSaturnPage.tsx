...
const config = comboConfigs['mercury-mars-jupiter-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '远征目标', icon: '🌌' },
  { id: 'm2', name: '持续行动', icon: '🌟' },
  { id: 'm3', name: '挫折转化', icon: '💫' },
  { id: 'm4', name: '里程碑', icon: '✨' },
]
...
export default function MercuryMarsJupiterSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mmjs-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mmjs-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mmjs-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mmjs-items4', [])
