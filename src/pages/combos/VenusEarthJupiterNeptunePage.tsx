...
const config = comboConfigs['venus-earth-jupiter-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '内心宝藏', icon: '🌌' },
  { id: 'm2', name: '灵性滋养', icon: '🌟' },
  { id: 'm3', name: '丰盛感受', icon: '💫' },
  { id: 'm4', name: '内在和平', icon: '✨' },
]
...
export default function VenusEarthJupiterNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vejn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vejn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vejn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vejn-items4', [])
