...
const config = comboConfigs['mercury-mars-jupiter-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '梦想地图', icon: '🌌' },
  { id: 'm2', name: '热情行动', icon: '🌟' },
  { id: 'm3', name: '信念支撑', icon: '💫' },
  { id: 'm4', name: '远方召唤', icon: '✨' },
]
...
export default function MercuryMarsJupiterNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mmjn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mmjn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mmjn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mmjn-items4', [])
