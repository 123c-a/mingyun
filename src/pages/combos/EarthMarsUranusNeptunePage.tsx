...
const config = comboConfigs['earth-mars-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '工作旧模式', icon: '🌌' },
  { id: 'm2', name: '转变时刻', icon: '🌟' },
  { id: 'm3', name: '新职业身份', icon: '💫' },
  { id: 'm4', name: '事业重生', icon: '✨' },
]
...
export default function EarthMarsUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('emun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('emun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('emun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('emun-items4', [])
