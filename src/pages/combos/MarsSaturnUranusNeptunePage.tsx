...
const config = comboConfigs['mars-saturn-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '旧力量模式', icon: '🌌' },
  { id: 'm2', name: '能量转化', icon: '🌟' },
  { id: 'm3', name: '新力量形态', icon: '💫' },
  { id: 'm4', name: '力量重生', icon: '✨' },
]
...
export default function MarsSaturnUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('msun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('msun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('msun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('msun-items4', [])
