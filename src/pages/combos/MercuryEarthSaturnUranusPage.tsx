...
const config = comboConfigs['mercury-earth-saturn-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '舒适区边界', icon: '🌌' },
  { id: 'm2', name: '小步突破', icon: '🌟' },
  { id: 'm3', name: '新身份', icon: '💫' },
  { id: 'm4', name: '成长记录', icon: '✨' },
]
...
export default function MercuryEarthSaturnUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mesu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mesu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mesu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mesu-items4', [])
