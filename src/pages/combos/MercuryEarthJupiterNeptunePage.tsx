...
const config = comboConfigs['mercury-earth-jupiter-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '直觉指引', icon: '🌌' },
  { id: 'm2', name: '沉浸式学习', icon: '🌟' },
  { id: 'm3', name: '灵感笔记', icon: '💫' },
  { id: 'm4', name: '智慧整合', icon: '✨' },
]
...
export default function MercuryEarthJupiterNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mejn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mejn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mejn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mejn-items4', [])
