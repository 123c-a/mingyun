...
const config = comboConfigs['venus-mars-saturn-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '旧关系模式', icon: '🌌' },
  { id: 'm2', name: '革命时刻', icon: '🌟' },
  { id: 'm3', name: '新关系范式', icon: '💫' },
  { id: 'm4', name: '关系进化', icon: '✨' },
]
...
export default function VenusMarsSaturnUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vmsu-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vmsu-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vmsu-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vmsu-items4', [])
