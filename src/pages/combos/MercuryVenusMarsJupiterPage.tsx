...
const config = comboConfigs['mercury-venus-mars-jupiter']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '热情话语', icon: '🌌' },
  { id: 'm2', name: '大胆表达', icon: '🌟' },
  { id: 'm3', name: '爱的扩张', icon: '💫' },
  { id: 'm4', name: '真实沟通', icon: '✨' },
]
...
export default function MercuryVenusMarsJupiterPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvmj-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvmj-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvmj-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvmj-items4', [])
