...
const config = comboConfigs['mercury-venus-mars-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '关系功课', icon: '🌌' },
  { id: 'm2', name: '耐心练习', icon: '🌟' },
  { id: 'm3', name: '承诺记录', icon: '💫' },
  { id: 'm4', name: '深度连接', icon: '✨' },
]
...
export default function MercuryVenusMarsSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvms-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvms-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvms-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvms-items4', [])
