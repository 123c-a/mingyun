...
const config = comboConfigs['venus-jupiter-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '内在丰盛', icon: '🌌' },
  { id: 'm2', name: '灵性财富', icon: '🌟' },
  { id: 'm3', name: '富足感受', icon: '💫' },
  { id: 'm4', name: '无尽给予', icon: '✨' },
]
...
export default function VenusJupiterSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vjsn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vjsn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vjsn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vjsn-items4', [])
