...
const config = comboConfigs['mercury-earth-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '灵感捕捉', icon: '🌌' },
  { id: 'm2', name: '现实转化', icon: '🌟' },
  { id: 'm3', name: '创意执行', icon: '💫' },
  { id: 'm4', name: '梦想现实', icon: '✨' },
]
...
export default function MercuryEarthUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('meun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('meun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('meun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('meun-items4', [])
