...
const config = comboConfigs['jupiter-saturn-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '时代觉察', icon: '🌌' },
  { id: 'm2', name: '意识进化', icon: '🌟' },
  { id: 'm3', name: '集体转变', icon: '💫' },
  { id: 'm4', name: '新时代', icon: '✨' },
]
...
export default function JupiterSaturnUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('jsun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('jsun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('jsun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('jsun-items4', [])
