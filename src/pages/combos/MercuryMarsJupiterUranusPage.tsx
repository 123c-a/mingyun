...
const config = comboConfigs['mercury-mars-jupiter-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '闪电创意', icon: '🌌' },
  { id: 'm2', name: '快速执行', icon: '🌟' },
  { id: 'm3', name: '突破思维', icon: '💫' },
  { id: 'm4', name: '创新成果', icon: '✨' },
]
...
export default function MercuryMarsJupiterUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mmju-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mmju-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mmju-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mmju-items4', [])
