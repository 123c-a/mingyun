...
const config = comboConfigs['mercury-venus-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '灵感涌现', icon: '🌌' },
  { id: 'm2', name: '创意流动', icon: '🌟' },
  { id: 'm3', name: '梦境创意', icon: '💫' },
  { id: 'm4', name: '直觉创造', icon: '✨' },
]
...
export default function MercuryVenusUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mvun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mvun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mvun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mvun-items4', [])
