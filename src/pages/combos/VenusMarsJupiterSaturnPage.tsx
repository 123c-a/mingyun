...
const config = comboConfigs['venus-mars-jupiter-saturn']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '长期热情', icon: '🌌' },
  { id: 'm2', name: '深度关系', icon: '🌟' },
  { id: 'm3', name: '共同成长', icon: '💫' },
  { id: 'm4', name: '承诺之火', icon: '✨' },
]
...
export default function VenusMarsJupiterSaturnPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vmjs-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vmjs-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vmjs-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vmjs-items4', [])
