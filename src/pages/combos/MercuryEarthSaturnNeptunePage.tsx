...
const config = comboConfigs['mercury-earth-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '深潜时刻', icon: '🌌' },
  { id: 'm2', name: '耐心等待', icon: '🌟' },
  { id: 'm3', name: '内在成长', icon: '💫' },
  { id: 'm4', name: '根基加固', icon: '✨' },
]
...
export default function MercuryEarthSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('mesn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('mesn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('mesn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('mesn-items4', [])
