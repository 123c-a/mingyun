...
const config = comboConfigs['mercury-earth-jupiter-uranus']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '新知探索', icon: '🌌' },
  { id: 'm2', name: '跨界连接', icon: '🌟' },
  { id: 'm3', name: '创新思维', icon: '💫' },
  { id: 'm4', name: '快速入门', icon: '✨' },
]
...
export default function MercuryEarthJupiterUranusPage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('meju-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('meju-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('meju-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('meju-items4', [])
