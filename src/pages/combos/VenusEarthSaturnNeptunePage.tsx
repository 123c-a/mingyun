...
const config = comboConfigs['venus-earth-saturn-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '深度连接', icon: '🌌' },
  { id: 'm2', name: '时间之爱', icon: '🌟' },
  { id: 'm3', name: '深情表达', icon: '💫' },
  { id: 'm4', name: '根基之爱', icon: '✨' },
]
...
export default function VenusEarthSaturnNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vesn-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vesn-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vesn-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vesn-items4', [])
