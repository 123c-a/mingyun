...
const config = comboConfigs['venus-saturn-uranus-neptune']
const PRIMARY_COLOR = config.primaryColor
const SECONDARY_COLOR = config.secondaryColor

const TAB_LIST = [
  { id: 'm1', name: '心的旧壳', icon: '🌌' },
  { id: 'm2', name: '瓦解时刻', icon: '🌟' },
  { id: 'm3', name: '心的重生', icon: '💫' },
  { id: 'm4', name: '爱的进化', icon: '✨' },
]
...
export default function VenusSaturnUranusNeptunePage() {
  const [activeTab, setActiveTab] = useState<string>('m1')
  const [items1, setItems1] = useLocalStorage<Item1[]>('vsun-items1', [])
  const [items2, setItems2] = useLocalStorage<Item2[]>('vsun-items2', [])
  const [items3, setItems3] = useLocalStorage<Item3[]>('vsun-items3', [])
  const [items4, setItems4] = useLocalStorage<Item4[]>('vsun-items4', [])
